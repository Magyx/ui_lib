...
mod paint {
    ...
    fn rectangle_emits_one_instance_with_correct_color() {
        ...
        let prim = instances[0].primitive();
    ...
    fn row_of_two_rectangles_emits_two_instances_in_order() {
        ...
        let p0 = instances[0].primitive();
        let p1 = instances[1].primitive();
    ...
    fn clip_children_sets_scissor_on_child_instances() {
        ...
        let scissor = instances[0].scissor();
        assert_eq!(
            scissor,
            Some([0, 0, 200, 100]),
    ...
    fn row_with_background_emits_bg_then_children() {
        ...
        assert_eq!(
            instances[0].primitive().data1[0],
            Color::RED.0,
            "first instance is the Row background"
        );
        assert_eq!(
            instances[1].primitive().data1[0],
            Color::GREEN.0,
    ...
    fn nested_row_column_positions_correct_in_paint_output() {
        ...
        let pa = instances[0].primitive();
        let pb = instances[1].primitive();
        let pc = instances[2].primitive();
