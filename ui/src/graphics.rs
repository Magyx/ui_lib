...
fn cascade_widgets<'a>(
    ...
    window_size: &[f32],
) -> (Option<Vec<Primitive>>, Option<Vec<Text<'a>>>) {
    let RenderOutput { primitives, texts } = widgets.as_primitive(Size {
...
impl<'a> Engine<'a> {
    ...
    pub fn view(&mut self, build: impl FnOnce(&Engine) -> Element) {
        let element = build(self);
        let (primitives, texts) = cascade_widgets(&element.widget, &self.globals.window_size);
    ...
    pub fn new(
        ...
        let render_pipeline_layout =
            config
                ...
                .create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                    label: Some(&format!("Primitive Render Pipeline Layout")),
                    bind_group_layouts: &[],
        ...
        let render_pipeline =
            config
                ...
                .create_render_pipeline(&wgpu::RenderPipelineDescriptor {
                    label: Some(&format!("Primitive Render Pipeline")),
        ...
        let vertex_buffer = config
            ...
            .create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some(&format!("Primitive Vertex Buffer")),
        ...
        let index_buffer = if indices.is_empty() {
            ...
            Some(
                config
                    .device
                    .create_buffer_init(&wgpu::util::BufferInitDescriptor {
                        label: Some(&format!("Primitive Index Buffer")),
        ...
        let instance_buffer = config.device.create_buffer(&wgpu::wgt::BufferDescriptor {
            label: Some(&format!("Primitive Instance Buffer")),
